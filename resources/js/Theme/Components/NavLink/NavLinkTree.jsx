...
export default function NavLinkTree({
  ...
  const team_slug = auth.user.role
  ...
  const isManualGuide =
    components.includes('manual-guide.book') || components.includes('manual-guide.video')

  if (canAccess || isDashboard || isProfile || isManualGuide)
    return (
      <Fragment>
        <button
          type="button"
      ...
      `}
        >
          ...
          <div
            className={`lg:hidden absolute z-10 flex-between w-full ml-4 px-4 rounded-full ${
              isActive && 'text-primary'
            }`}
          >
            &nbsp;&nbsp;
            {label}
            <div className="flex-end">{expand ? <FaChevronDown /> : <FaChevronUp />}</div>
          </div>
...
const NavChild = ({ child_route, current_route, theme, label, team_slug, icon }) => {
  ...
  const isManualGuide =
    child_route == 'manual-guide.book' || child_route == 'manual-guide.video'
...
`}
      >
        ...
        <div
          className={`lg:hidden absolute z-50 flex-between ml-4 px-4 rounded-full ${
